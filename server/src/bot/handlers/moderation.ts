...
import { updateQuizStatus, getQuizById, getProfileByTelegramId, updatePersonalityTestStatus, getPersonalityTestById } from '../../lib/supabase.js';
...
export function registerModerationHandlers(bot: Bot<Context>): void {
  ...
  // ===============================
  // Personality Test Moderation
  // ===============================

  // Handle approve_personality_test callback
  bot.callbackQuery(/^approve_personality_test:(.+)$/, async (ctx) => {
    const userId = ctx.from?.id;
    if (!userId || !ADMIN_TELEGRAM_IDS.includes(userId)) {
      await ctx.answerCallbackQuery({ text: '❌ Только админы могут модерировать', show_alert: true });
      return;
    ...
    try {
      const test = await updatePersonalityTestStatus(testId, true);

      if (!test) {
        await ctx.answerCallbackQuery({ text: '❌ Тест не найден', show_alert: true });
        return;
      }

      // Notify author
      if (test.created_by) {
        const profile = await getProfileByTelegramId(parseInt(test.created_by));
        if (profile?.telegram_id) {
          await notifyAuthorModerationResult(profile.telegram_id, test.title, true);
        }
      }
      ...
      await ctx.answerCallbackQuery({ text: '✅ Тест опубликован!' });
    } catch (error) {
      console.error('Approve personality test error:', error);
      await ctx.answerCallbackQuery({ text: '❌ Ошибка при одобрении', show_alert: true });
    }
  });

  // Handle reject_personality_test callback
  bot.callbackQuery(/^reject_personality_test:(.+)$/, async (ctx) => {
    const userId = ctx.from?.id;
    if (!userId || !ADMIN_TELEGRAM_IDS.includes(userId)) {
      await ctx.answerCallbackQuery({ text: '❌ Только админы могут модерировать', show_alert: true });
      return;
    ...
    // Ask for rejection reason
    const keyboard = new InlineKeyboard()
      .text('🚫 Некачественный контент', `reject_test_reason:${testId}:quality`)
      .row()
      .text('⚠️ Нарушение правил', `reject_test_reason:${testId}:rules`)
      .row()
      .text('❓ Дублирующий контент', `reject_test_reason:${testId}:duplicate`)
      .row()
      .text('📝 Другая причина', `reject_test_reason:${testId}:other`)
      .row()
      .text('❌ Отмена', `cancel_test_reject:${testId}`);

    await ctx.editMessageReplyMarkup({ reply_markup: keyboard });
    await ctx.answerCallbackQuery();
  });

  // Handle test rejection reason selection
  bot.callbackQuery(/^reject_test_reason:(.+):(.+)$/, async (ctx) => {
    const userId = ctx.from?.id;
    if (!userId || !ADMIN_TELEGRAM_IDS.includes(userId)) {
      await ctx.answerCallbackQuery({ text: '❌ Только админы могут модерировать', show_alert: true });
      return;
    ...
    const reason = reasons[reasonCode] || 'Тест не прошёл модерацию';

    try {
      const test = await updatePersonalityTestStatus(testId, false);
      ...
      if (test.created_by) {
        const profile = await getProfileByTelegramId(parseInt(test.created_by));
        if (profile?.telegram_id) {
          await notifyAuthorModerationResult(profile.telegram_id, test.title, false, reason);
        }
      }
      ...
      await ctx.answerCallbackQuery({ text: '❌ Тест отклонён' });
    } catch (error) {
      console.error('Reject personality test error:', error);
      await ctx.answerCallbackQuery({ text: '❌ Ошибка при отклонении', show_alert: true });
    }
  });

  // Handle cancel test rejection
  bot.callbackQuery(/^cancel_test_reject:(.+)$/, async (ctx) => {
    const testId = ctx.match[1];

    // Restore original buttons
    const keyboard = new InlineKeyboard()
      .text('✅ Одобрить', `approve_personality_test:${testId}`)
      .text('❌ Отклонить', `reject_personality_test:${testId}`);
